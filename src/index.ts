...
function parseProbeInfo(track: Pick<TrackInfo, "probeInfo">, input: DataInput) {
  ...
  const name =
    separatorPosition < 0
      ? probeInfo
      : probeInfo.substring(0, separatorPosition);
  const parameters =
    separatorPosition < 0 ? null : probeInfo.substring(separatorPosition + 1);
  track.probeInfo = { raw: probeInfo, name, parameters };
}

function writeProbeInfo(
  track: Pick<TrackInfo, "probeInfo">,
  output: DataOutput
) {
...
export function decode(data: Uint8Array | string): TrackInfo {
  ...
  if (!decoder) {
    throw new Error(
      "This track's version is not supported. Track version: " +
        version +
        ", supported versions: " +
        decoders
          .filter((e) => e)
...
export function encode(
  ...
  version: number = TRACK_INFO_VERSION
): string {
